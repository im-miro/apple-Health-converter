...
func main() {
	...
	bodyMassData := map[string]float64{}
	counts := map[string]int{}

	for _, rec := range root.Records {
		if rec.Type != "HKQuantityTypeIdentifierBodyMass" {
			continue
		}
		...
		bodyMassData[date] += val
		counts[date]++
	}

	var dates []string
	for d := range bodyMassData {
	...
	for _, d := range dates {
		...
		avg := bodyMassData[d] / float64(counts[d])
		yValues = append(yValues, avg)
	}

	graph := chart.Chart{
		Title:      "Body Mass Trend",
	...
	f, err := os.Create("bodymass_chart.png")
	if err != nil {
	...
	log.Println("✅ bodymass_chart.png を生成しました")
}
