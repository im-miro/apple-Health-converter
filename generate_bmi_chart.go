...
func main() {
	...
	bmiData := map[string]float64{}
	counts := map[string]int{}

	for _, rec := range root.Records {
		if rec.Type != "HKQuantityTypeIdentifierBodyMassIndex" {
			continue
		}
	...
	for _, d := range dates {
		...
		avg := bmiData[d] / float64(counts[d])
	...
	log.Println("✅ bmi_chart.png を生成しました")
}
